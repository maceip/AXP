...
def load_fonts(font_family: Optional[str] = None) -> Optional[dict]:
    ...
    # AXP: a local font directory (AXP_POSTER_FONT_DIR or ./fonts) holding
    # bold/regular/light files, e.g. AXP Runde built by
    # scripts/fonts/build-axp-runde.py. Roboto is not vendored; without local
    # files matplotlib falls back to its bundled DejaVu Sans.
    font_dir = os.environ.get("AXP_POSTER_FONT_DIR", FONTS_DIR)
    candidates = {
    ...
    for weight, names in candidates.items():
        for name in names:
            ...
            if os.path.exists(path):
                ...
                break
    if len(fonts) < 3:
        print(f"⚠ No complete font set in {font_dir}; using matplotlib's DejaVu Sans")
        return None
    return fonts
