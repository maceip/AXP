...
export function buildStyleBlock(font: string, hasMonoFont: boolean): string {
  // AXP: no remote @import. The SVG is inlined into a page that self-hosts its
  // fonts, and the workspace never loads third-party resources.
  ...
  return [
    ...
    /* AXP kawaii treatment: sticker-soft nodes, round joins everywhere. */
    `  .node > :first-child { filter: url(#axp-soft); }`,
    `  .edge, .node *, .subgraph * { stroke-linejoin: round; stroke-linecap: round; }`,
    `  svg {${derivedVars}`,
    `  }`,
    '</style>',
  ].join('\n')
